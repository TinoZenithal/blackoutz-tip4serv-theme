  ...
  return <>
    ...
    <div className="product-grid">
      {visibleProducts.map((product) => <article className="product" key={product.id}>
        ...
        <div className="product-info"><p className="product-tag">{product.tag}</p><h3>{product.name}</h3><p>{product.description}</p>{product.details && product.details.length > 0 && <details className="product-details"><summary>VIEW DETAILS <span aria-hidden="true">+</span></summary><ul>{product.details.map((detail) => <li key={detail}>{detail}</li>)}</ul></details>}<small className="product-legal">{product.customAmount ? 'INCLUDES A BLACKOUTZ SUPPORTER ROLE • NO CASH VALUE' : 'NON-REDEEMABLE DIGITAL REWARD • NO REAL-WORLD VALUE'}</small>
      ...
      <section ref={dialogRef} className={`checkout-modal checkout-step-${checkoutStep}`} role="dialog" aria-modal="true" aria-labelledby="checkout-title" aria-describedby="checkout-description">
        ...
        <div className="checkout-modal-content">
          ...
          {checkoutStep === 'review' && <form onSubmit={continueToVerification}>
            ...
            <label className="confirm"><input type="checkbox" checked={reviewConfirmed} onChange={(event) => setReviewConfirmed(event.target.checked)} required/><span>{selected.customAmount ? 'I understand this supporter purchase includes a digital BLACKOUTZ community role, has no cash value and is not refundable for change of mind.' : selected.billing === 'monthly' ? 'I understand this is a recurring monthly digital subscription and earlier charges are not refundable for change of mind.' : 'I understand this is a non-redeemable digital in-game reward and is not refundable for change of mind.'} Statutory consumer rights are not excluded.</span></label>
