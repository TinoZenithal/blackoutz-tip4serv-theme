...
- Fully custom three-stage order review, player verification and payment-review interface
- Secure Discord OAuth linking with no manual numeric Discord ID field
...
When a customer is ready to buy, the custom BLACKOUTZ checkout keeps the order review, receipt email, in-game username, Discord link and final payment review inside this storefront. Discord linking uses the official OAuth authorization-code flow. The Discord ID is kept in a signed HTTP-only session and is never exposed in the browser interface.

After the customer confirms the final review, the browser calls this storefront's server-side checkout route. The route combines the verified Discord identity with the customer-entered fields and asks Tip4Serv to create the checkout using the private server-side API key. It sends `addtocart` for one-time products, `subscribe` for monthly products and the selected supported currency. Tip4Serv or its payment provider then supplies the protected payment controls; BLACKOUTZ never collects card or PayPal credentials.
...
Copy `.env.example` to `.env.local` and provide `TIP4SERV_API_KEY`, `DISCORD_CLIENT_ID` and `DISCORD_CLIENT_SECRET`. Never prefix private values with `NEXT_PUBLIC_` or commit real credentials. The keys are used only by server routes. The store, enabled products, categories and Tip4Serv product IDs are discovered automatically. Optional ID overrides support diagnostics and the branded fallback catalogue.

The live checkout currently requires `email`, `discord_id` and `ingame_username` because of the store's Tip4Serv fulfilment configuration. Customers do not type a raw Discord ID: they authorize the dedicated BLACKOUTZ Discord application, and the server supplies the verified ID to Tip4Serv's configured role automation.

Set `SITE_URL` to the deployed storefront address so Discord can return from OAuth and Tip4Serv can return customers to the correct success, pending or cancelled state. Add both the production and localhost callback URLs shown in `.env.example` setup to the Discord Developer Portal OAuth2 redirects.
...
Tip4Serv's API Themes area links to self-hosted demos and source repositories. This Next.js project is the self-hosted API theme. The custom theme now handles product review and player verification before handing only the protected payment portion to Tip4Serv. Listing this storefront inside the Tip4Serv API Themes area still requires Tip4Serv's review and approval.
